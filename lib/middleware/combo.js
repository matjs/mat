  ...
  return function *combohandler(next){
    ...
    if (
      ...
      for (var i = 0; i < urls.length; i++) {
        ...
        } else if (this.proxyHeaders['content-encoding'] == 'gzip') {
          // 返回的内容被gzip过需要ungzip，不然有可能乱码
          body = yield mutil.cozip(this.body)
        } else {
          body = this.body
        }
      ...
      if (this.proxyHeaders) {
        for (var i = 0; i < headerKey.length; i++) {
          if (this.proxyHeaders[headerKey[i]]) {
            this.set(headerKey[i], this.proxyHeaders[headerKey[i]])
          }
        }
      }
