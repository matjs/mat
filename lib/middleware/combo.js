...
module.exports = function combohandler (gen) {
  return function * combohandler (next) {
    ...
    // 检查是否为 SSE 请求，如果是则直接跳过 combo 处理
    const acceptHeader = this.headers.accept || ''
    if (acceptHeader.includes('text/event-stream') || this.url.includes('/sse/') || this.url.includes('/events/')) {
    ...
    if (combohandler && match(rules, this.url) && this.url.indexOf(opts.trigger) !== -1) {
      ...
      for (let i = 0; i < urls.length; i++) {
        ...
        // 检查是否为 SSE 响应，如果是则不进行转换
        if (this.state.resHeaders && mutil.isSSE(this.state.resHeaders)) {
          body = this.body
        } else if (this.body && this.body instanceof Stream) {
