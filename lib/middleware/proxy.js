...
module.exports = function proxy(timeout, limit) {
  return function* proxy(next) {
    ...
    if (isJson) {
      ...
      //修正content-length
      options.headers['content-length'] = Buffer.byteLength(qs.stringify(parsedBody))
      ...
      for (let k in parsedBody) {
        let vType = Object.prototype.toString.call(parsedBody[k])
