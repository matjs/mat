...
const proxySSE = require('./proxy-sse')
...
module.exports = function proxy (timeout, limit) {
  return function * proxy (next) {
    ...
    // SSE 检测和处理分支 - 添加详细调试
    console.log('[MAT-PROXY] Checking SSE request:', {
      url: this.url,
      method: this.method,
    ...
    // console.log('[MAT-PROXY] All request headers:', this.headers)
    
    const isSSERequest = sseDetector.isSSERequest(this, this.sseConfig)
    console.log('[MAT-PROXY] SSE detection result:', isSSERequest)
    
    if (isSSERequest) {
      console.log('[MAT-PROXY] SSE request detected, using SSE proxy handler (passthrough mode)')
      ...
      console.log('[MAT-PROXY] Building SSE target URL:', {
        protocol: this.protocolAlias || this.protocol,
        host: this.proxyPass || this.host,
        pathname: this.path,
        search: this.search,
      ...
      const sseProxyOptions = {
        ...
        // 传递代理配置
        isChangeOrigin: this.isChangeOrigin !== undefined ? this.isChangeOrigin : true, // 默认启用
        protocolAlias: this.protocolAlias,
        rapProjectId: this.rapProjectId
      }
      
      // 使用 SSE 专用处理器
      yield proxySSE.handleSSEProxy(this, sseProxyOptions, this.sseConfig)
      
      // SSE 处理完成，直接返回，不继续执行后续逻辑
      return
    }
