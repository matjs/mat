...
module.exports = function proxy(timeout, limit) {
  return function* proxy(next) {
    ...
    if (!/^http/.test(this.url) && !this.proxyPass) {
      console.log('mat proxy >>>>>>', this.url, 'no proxyPass', this.proxyPass)
      return;
    }
    ...
    if (isSSERequest) {
      console.log(
        '[MAT-PROXY] SSE request detected, using SSE proxy handler (passthrough mode)',
      );
      ...
      const sseProxyOptions = {
        ...
        timeout: 0, // SSE连接不设置超时
        // 传递代理配置
        isChangeOrigin:
          this.isChangeOrigin !== undefined ? this.isChangeOrigin : true, // 默认启用
    ...
    if (this.headers.origin && this.protocolAlias) {
      this.headers.origin = this.headers.origin.replace(
        /https?/,
        this.protocolAlias,
      );
    }
